<?php
     ...
     if($conn->connect_error){
         ...
         $stmt = $conn->prepare("insert into online_payment(PStudName,PPhn,PAPhn,PEmail,PIdno,PClass)values(?, ?, ?, ?, ?, ?)");
         $stmt->bind_param("siisis",$PStudName,$PPhn,$PAPhn,$PEmail,$PIdno,$PClass);
         $stmt-> execute();
         $stmt->close();
         $conn->close();
     }
